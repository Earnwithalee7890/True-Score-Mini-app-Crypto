...
export function TrueScoreApp() {
  ...
  // Helper to update user data and detect weekly score changes
  const updateUserData = (data: any) => {
    ...
    if (lastQuotient !== -1 && lastQuotient !== newQuotient) {
      ...
      const now = Date.now()
      // Notify only if a week has passed since last notification
  ...
  // Initialize SDK and fetch data on mount
  useEffect(() => {
    const init = async () => {
      try {
        const frameContext = await sdk.context
        setContext(frameContext)
        // Apply theme from Farcaster client if available
        ...
        // Show add‑app prompt for first‑time users (no fid in context)
        if (!frameContext?.user?.fid) {
          setShowAddPrompt(true)
        }
        // Fetch user data (fallback fid = 338060)
  ...
  return (
    <main className="min-h-screen bg-background px-4 py-8 overflow-hidden">
      ...
      {showScoreUpdate && (
        <div className="fixed top-4 left-1/2 -translate-x-1/2 bg-primary/10 border border-primary/30 text-primary-foreground px-4 py-2 rounded-md shadow-md animate-fade-in">
